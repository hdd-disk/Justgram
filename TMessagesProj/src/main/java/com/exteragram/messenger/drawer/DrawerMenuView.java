...
public class DrawerMenuView extends ScrollView {
    ...
    public void rebuildMenu(int i, BaseFragment baseFragment) {
        ...
        for (int i2 = 0; i2 < ExteraConfig.getMainMenuLayout().size(); i2++) {
            ...
            if (num != MainMenuItem.DIVIDER.getId()) {
                List<MainMenuHelper.MenuItemInfo> listResolveDrawerMenuItems = MainMenuHelper.resolveDrawerMenuItems(num, menuContextCreateMenuContext);
                if (!listResolveDrawerMenuItems.isEmpty()) {
                    ...
                    for (final MainMenuHelper.MenuItemInfo menuItemInfo : listResolveDrawerMenuItems) {
                        ...
                        drawerMenuItemView.setMenuItem(num, i, menuItemInfo.iconRes(), menuItemInfo.text());
                        drawerMenuItemView.setOnClickListener(view2 -> onMenuItemClicked(menuItemInfo, view2));
                        if (menuItemInfo.onLongClick() != null) {
                            drawerMenuItemView.setOnLongClickListener(view2 -> onMenuItemLongClicked(menuItemInfo, view2));
                        }
    ...
    private void onMenuItemClicked(MainMenuHelper.MenuItemInfo menuItemInfo, View view) {
    ...
    private boolean onMenuItemLongClicked(MainMenuHelper.MenuItemInfo menuItemInfo, View view) {
        Runnable runnable = this.onItemClick;
        if (runnable != null) {
            runnable.run();
        }
    ...
    public void updateColors() {
        for (int i = 0; i < this.container.getChildCount(); i++) {
            ...
            if (childAt instanceof DrawerMenuItemView) {
                ...
                childAt.setBackgroundColor(Theme.getColor(Theme.key_divider));
            }
        }
        updateGradient();
    }
    ...
    private void updateGradient() {
        ...
        if (this.topGradient == null || color != this.lastGradientColor) {
            ...
            LinearGradient linearGradient = new LinearGradient(0.0f, 0.0f, 0.0f, AndroidUtilities.dp(16.0f), new int[]{color, 16777215 & color}, null, Shader.TileMode.CLAMP);
