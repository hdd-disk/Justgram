    ...
    public InstantCameraVideoEncoderOverlayHelper(int width, int height) {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            if (i == TEXTURE_INDEX_WATERMARK_LOGO) {
                ...
                final TLottieNative tLottie = TLottieNative.createFromRawJson(AndroidUtilities.readRes(R.raw.plane_logo_plain), "logo_plane", null);
                ...
                for (int x = 0; x < 8; x++) {
                    for (int y = 0; y < 4; y++) {
                        ...
                        tLottie.getFrame(index * 2, logoBitmap, true);
